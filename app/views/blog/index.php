...
<body class="bg-gray-100 min-h-screen">
    ...
    <nav class="bg-slate-800 text-white shadow-lg">
        <div class="max-w-7xl mx-auto px-4 py-4 flex justify-between items-center">
            <a href="vehicule" class="text-2xl font-bold">MaBagnole 🚗</a>
            <div class="space-x-6">
                <a href="../vehicule/index" class="hover:text-blue-400">Location</a>
                <a href="blog" class="text-blue-400 font-semibold">Blog</a>
                <?php if(isset($_SESSION['user_id'])): ?>
                    <a href="logout" class="bg-red-500 px-4 py-2 rounded hover:bg-red-600">Déconnexion</a>
                <?php else: ?>
                    <a href="login" class="bg-blue-600 px-4 py-2 rounded hover:bg-blue-700">Connexion</a>
                <?php endif; ?>
            </div>
        </div>
    </nav>
    ...
    <div class="max-w-7xl mx-auto px-4 py-8 grid grid-cols-1 md:grid-cols-4 gap-8">
        ...
        <aside class="space-y-6">
            <div class="bg-white p-5 rounded-xl shadow-sm">
                ...
                <ul class="space-y-2">
                    <li><a href="../blog/index" class="text-gray-600 hover:text-blue-600">Tous les thèmes</a></li>
                    <?php foreach($themes as $t): ?>
                        <li>
                            <a href="blog?theme=<?= $t['id_theme'] ?>" class="block p-2 rounded <?= $theme_id == $t['id_theme'] ? 'bg-blue-100 text-blue-700' : 'text-gray-600 hover:bg-gray-50' ?>">
            ...
            <div class="bg-white p-5 rounded-xl shadow-sm">
                ...
                <div class="flex flex-wrap gap-2">
                    <?php foreach($tags as $tag): ?>
                        <a href="blog?tag=<?= $tag['id_tag'] ?>" class="text-xs bg-gray-200 px-2 py-1 rounded hover:bg-blue-500 hover:text-white transition">
        ...
        <main class="md:col-span-3 space-y-6">
            ...
            <div class="flex flex-col md:flex-row gap-4 justify-between items-center bg-white p-4 rounded-xl shadow-sm">
                <form action="blog" method="GET" class="flex w-full md:w-2/3">
                ...
                <div class="flex items-center gap-4">
                    <select onchange="location = this.value;" class="border rounded-lg px-2 py-2 bg-gray-50">
                        <option value="blog?limit=5" <?= $limit==5 ? 'selected':'' ?>>5 / page</option>
                        <option value="blog?limit=10" <?= $limit==10 ? 'selected':'' ?>>10 / page</option>
                        <option value="blog?limit=15" <?= $limit==15 ? 'selected':'' ?>>15 / page</option>
                    </select>
                    <?php if(isset($_SESSION['user_id'])): ?>
                        <a href="../blog/index/ajouter" class="bg-green-600 text-white px-4 py-2 rounded-lg font-bold hover:bg-green-700">+ Article</a>
                    <?php endif; ?>
                </div>
            </div>
                        ...
                        <div class="bg-white rounded-xl shadow-sm overflow-hidden flex flex-col md:flex-row border border-transparent hover:border-blue-300 transition">
                            ...
                            <div class="p-6 flex-1">
                                ...
                                <div class="mt-4 flex justify-between items-center">
                                    ...
                                    <a href="../blog/index/detail?id=<?= $a['id_article'] ?>" class="text-blue-600 font-bold hover:underline">Lire la suite →</a>
                                </div>
                            </div>
                        </div>
                    <?php endforeach; ?>
            ...
            <?php if($totalPages > 1): ?>
                <div class="flex justify-center space-x-2 py-4">
                    <?php for($i=1; $i<=$totalPages; $i++): ?>
                        <a href="blog?page=<?= $i ?>&limit=<?= $limit ?>&theme=<?= $theme_id ?>" 
